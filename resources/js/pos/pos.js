...
function addItemToCart(item) {
    ...
    if (existingItem) {
        ...
        let itemVat = item.tax_active ? (item.price1 * (item.tax_percentage / 100)) : 0;
        ...
        cart.push({
            ...
            qty: 1,
            vat: itemVat,
            total: item.price1 // الإجمالي يشمل الضريبة في الأنظمة الخليجية عادة
        });
    }
    renderCart();
}
...
// 4. قارئ الباركود المخفي والمفعل دائماً (Global Key Listener)
let barcodeBuffer = '';
let barcodeTimeout;
...
// 5. فلترة الأزرار السريعة بالتصنيف
...
// 7. إرسال الفاتورة للسيرفر (Save & Print)
function submitInvoice(isWaiting = false) {
    ...
    .then(data => {
        if (data.success) {
            ...
            // هنا يمكن إضافة كود لفتح نافذة طباعة الإيصال الحراري
        } else {
            alert(data.message);
        }
    });
